...
export function useEventManager(parameters: {
  ...
  const loadEvent = useCallback(async (eventId: number) => {
    ...
    try {
      ...
      const handle = await c.getEncryptedCount(eventId);
