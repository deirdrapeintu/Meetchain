...
export async function publicKeyStorageGet(aclAddress: `0x${string}`): Promise<{ publicKey?: { data: Uint8Array | null; id: string | null }; publicParams: { "2048": { publicParamsId: string; publicParams: Uint8Array } } | null; }> {
  ...
  const publicKeyData = storedPublicKey?.publicKey; const publicKeyId = storedPublicKey?.publicKeyId;
  const publicParams = storedPublicParams ? { "2048": storedPublicParams } : null;
  const publicKey = publicKeyId && publicKeyData ? { id: publicKeyId, data: publicKeyData } : undefined;
  return { ...(publicKey !== undefined && { publicKey }), publicParams };
}

export async function publicKeyStorageSet(aclAddress: `0x${string}`, publicKey: FhevmStoredPublicKey | null, publicParams: FhevmStoredPublicParams | null) {
